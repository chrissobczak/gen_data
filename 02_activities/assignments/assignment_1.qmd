...
```{r}
geno_subset <- data.table::fread('../data/gwa_qc_additive_A1.raw')
geno_subset <- na.omit(geno_subset[,c(
	'PHENOTYPE',
	'rs1861_C'
)])
...
summary(model)
# The p-value is <2.2e-16, so significant
```
...
```{r}
coefficients(model)
# The coefficient for rs1861_C is 0.97382409 (p<2.2e-16)
# For each additional copy of the C allele is associated with an
# increase of 0.9738 units of the phenotype starting at
# the estimate intercept of 0.0524 (where no allele C is present)
```
...
library(ggplot2)
data.frame(
	yhat=as.numeric(predict(model,geno_subset)),
	rs1861_C=geno_subset$rs1861_C
) -> yhat
...
geom_line(
	data=yhat,
	aes(x=rs1861_C,y=yhat),
	color='red',
	linewidth=2,
	alpha=.5
) +
labs(
	title='Regression under additive model of rs1861_C against a phenotype'
) +
theme_minimal()
```
...
```{r}
geno_rec <- geno_subset
geno_rec$rs1861_C <- ifelse(geno_rec$rs1861_C==2,1,0)
```
...
```{r}
model <- glm(PHENOTYPE ~ rs1861_C,data=geno_rec)
summary(model)
# The p-value is also <2.e-16, the intercept comes up to significant
# The AIC slightly increases
```
...
```{r}
data.frame(
	yhat=as.numeric(predict(model,geno_rec)),
	rs1861_C=geno_rec$rs1861_C
) -> yhat
...
labs(
	title='Regression under recessive model of rs1861_C against a phenotype'
) +
theme_minimal()
```
...
The recessive model fits better since AIC is higher (11292 v 11277)
