...
```{r}
fam <- data.table::fread('../data/gwa.A2.fam')
nrow(fam)
```
The dataset contains `r nrow(fam)` samples.

(ii) Read the `.bim` file. How many SNPs does the dataset contain?

```{r}
bim <- data.table::fread('../data/gwa.A2.bim')
nrow(bim)
```
The .bim file dataset contains `r nrow(bim)` SNPs.

(iii) Make a histogram (or density plot) of the phenotype. Does it look roughly Gaussian?

```{r,message=FALSE,warning=FALSE}
library(ggplot2)
ggplot(data=fam,aes(x=V6)) +
geom_histogram(aes(y=after_stat(density)),fill='#8A9A5B') +
geom_density() +
theme_minimal() +
labs(
	title='Histogram of Phenotype',
	x='Phenotype',
	y='Density'
)
```

The phenotype does look roughly Gaussian, it's symmetric and even
looks centered around zero with tails that look standard
(rare observations above and below 3 and -3).

#### Question 2: Quality control (QC)

Now we will perform QC using PLINK2 for the genotype files in `gwa.A2`.

(i) Using PLINK2 from the command line (bash), perform basic QC with the
following filters: MAF ≥ 0.05, SNP missingness (`--geno`) ≤ 0.01, individual
missingness (`--mind`) ≤ 0.10, and HWE p-value ≥ 0.00005, and output the QC’ed
dataset as `gwa.qc.A2`.
...
In this question, you will use **PLINK2’s built-in KING-robust kinship**
(`--king-cutoff`) detect and remove related individuals.

i)  Perform LD pruning on `gwa.qc.A2` using PLINK2 with the following
parameters: `--indep-pairwise 500 50 0.05`, and then generate a new dataset
containing only the pruned SNPs.

```{bash,results='hide',message=FALSE,warning=FALSE}
plink2 \
	--bfile ../data/gwa.qc.A2 \
	--indep-pairwise 500 50 0.05 \
	--out ../data/gwa.qc.A2
	
plink2 \
	--bfile ../data/gwa.qc.A2 \
	--extract ../data/gwa.qc.A2.prune.in \
	--make-bed \
	--out ../data/gwa.qc.A2.pruned
```

(ii) Use PLINK2 on the LD-pruned dataset to identify a set of unrelated
individuals up to (approximately) 2nd-degree relatives (use a kinship cutoff
of 0.0884).

```{bash,results='hide',message=FALSE,warning=FALSE}
plink2 \
	--bfile ../data/gwa.qc.A2.pruned \
	--king-cutoff 0.0884 \
	--out ../data/gwa_qc_A2
```

(iii) Using the unrelated individual list from part (ii), create a dataset
containing only unrelated individuals and name it `gwa.qc_unrelated.A2`.

```{bash,results='hide',message=FALSE,warning=FALSE}
plink2 \
	--bfile ../data/gwa.qc.A2 \
	--keep ../data/gwa_qc_A2.king.cutoff.in.id \
	--make-bed \
	--out ../data/gwa_qc_A2_unrelated
```

#### Question 4: principal component analysis (PCA)

(i) Using PLINK2, perform PCA on the unrelated, LD-pruned dataset to obtain
principal components.

    (Hint: Refer to the PCA section in the GWAS Tutorial II. You should use
    a `*.prune.in` file to select a set of independent SNPs before performing
    PCA.)

```{bash,results='hide',message=FALSE,warning=FALSE}
plink2 \
	--bfile ../data/gwa_qc_A2_unrelated \
	--extract ../data/gwa.qc.A2.prune.in \
	--make-bed \
	--out ../data/gwa_qc_A2_unrelated_pruned

plink2 \
	--bfile ../data/gwa_qc_A2_unrelated_pruned \
	--pca 20 \
	--out ../data/gwa_unrelated_PCA
```

#### Question 5: GWAS analyses

We now test for association between each SNP and the continuous phenotype,
adjusting for population structure using the top 3 PCs.

Assume that:

-   You will run GWAS on the unrelated, QC’ed dataset `gwa.qc_unrelated.A2`.

-   You have a covariate file (e.g., the `*.eigenvec` output from the PCA
analysis) containing PC1–PC3 for each individual.

(i) Using PLINK2, run a linear regression GWAS using `gwa.qc_unrelated.A2` as
the input dataset. Adjust for PCs 1–3 as covariates.
...
plink2 \
	...
	--ci 0.95 \
	--glm \
	--covar ../data/gwa_unrelated_PCA.eigenvec \
	--covar-name PC1,PC2,PC3 \
	--adjust \
	--out ../data/gwa_qc_A2_assoc
```

(ii) Create a Manhattan plot of the GWAS results.

```{r}
assoc <- fread('../data/gwa_qc_A2_assoc.PHENO1.glm.linear')
assoc_adjusted <- fread('../data/gwa_qc_A2_assoc.PHENO1.glm.linear.adjusted')
...
```{r}
png('qqplot.png',width=1200,height=800,res=150)
